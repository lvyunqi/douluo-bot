...
- 角色创建、武魂觉醒、每日签到、金魂币钱包、角色状态和当前位置查询。
...
| `武魂觉醒` | `觉醒` | 觉醒第一武魂 |
| `签到` | `每日签到`、`打卡` | 领取每日经验和金魂币；北京时间 04:00 刷新 |
| `钱包` | `我的钱包`、`余额` | 查看当前金魂币余额 |
| `状态` | `我的状态`、`属性` | 查看角色状态 |
...
签到以 `Asia/Shanghai` 每日 04:00 为游戏日边界。真实连签按相邻游戏日计算，漏签后从第 1 天重新开始；经验奖励按 7 日循环为 `60/70/80/90/100/110/150`，每次另得 `100-199` 金魂币。经验、钱包余额、领取记录和成功操作日志在同一个 SQLite 事务中提交，同一游戏日的重复请求不会再次发奖。
